...
{
  std::swap(head,x.head);//swapping all private members of this and input List
...
{
  return numnodes;//just returns length of List
}
...
{//******doesn't account for removing from the end
  ...
  else if(position==numnodes-1)     //debug fixing for the case that it is the final entry
  {                                  
  Node<T> *temp=*head;             //incrementing pointer starting from head pointer
  while (temp!=NULL)
  {                                  
    temp=temp->getNext();        //incrementing temp through 
    if(temp->getNext()==NULL)
      {                              // If the next temp doesn't exit
        temp=NULL;                 //set temp back to null
        break;
      }
    } 
  }
...
template <typename T>
void List<T>::clear()//as of 2:44pm Monday, my clear isn't working
{
...
// template <typename T>
// void List<T>::sort()//there is an error with multiple instances with the same value!!
...
// }
// template <typename T>
// List<T> List<T>::sort(List<T> list)
// {//bubblesort implementation 
// //**NO INSTRUCTION FOR RECURSION REQUIRED**
